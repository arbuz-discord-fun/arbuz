...
export function getChannel(): TextChannel | null {
  const ch = client.channels.cache.get(CHANNEL_ID);
  return (ch instanceof Object && 'send' in ch) ? ch as TextChannel : null;
}
...
client.on('messageCreate', async (message) => {
  ...
  try {
    if (cmd === '!arbuz')    return void await handleArbuz(message);
    if (cmd === '!да')       return void await handleDa(message);
    if (cmd === '!нет')      return void await handleNet(message);
    if (cmd === '!top')      return void await handleTop(message);
    if (cmd === '!setrole')  return void await handleSetrole(message);
  } catch (err) {

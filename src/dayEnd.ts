...
import {
  anarchyTwentyBlocksRole,
  finalizePendingAsOnes,
  topCandidates,
...
let _getChannel: (() => TextChannel | null) | null = null;

export function setChannelGetter(fn: () => TextChannel | null): void {
  _getChannel = fn;
}

function ch(): TextChannel | null {
...
export async function processDayEnd(date: string): Promise<void> {
  ...
  if (await anarchyTwentyBlocksRole(date)) {
    ...
    channel?.send(
      `🚫 Итоги за **${date}**: в арбузной анархии выпало **20**.\n` +
  ...
  if (!winners.length) {
    await setTykvenetsToAll();
    channel?.send(
      `📭 Итоги за **${date}**: никто не сделал обычный бросок \`!arbuz\`.\n` +
      `🎃 Тыквенцы у власти!`,
    );
    return;
  }
  ...
  if (winners.length === 1) {
    ...
    channel?.send(
      `🍉 Итоги за **${date}**: победитель ${mention(winnerId)} со значением **${maxVal}**.\n` +
  ...
  channel?.send(
    `⚔️ Итоги за **${date}**: ничья на **${maxVal}** между ` +
 ...
 */
export async function maybeFinishTiebreak(date: string): Promise<void> {
  ...
  if (res.length < users.length) return; // ещё не все бросили
  ...
  if (roundWinners.length === 1) {
    ...
    channel?.send(
      `✅ Тай-брейк за **${date}**, раунд ${round} завершён.\n` +
    ...
    await setTykvenetsToAll();
    channel?.send(
      `🤝 Ничья в тай-брейке за **${date}**, раунд ${round}!\n` +

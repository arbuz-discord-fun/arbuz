import { todayStr, formatMsk } from '../utils.js';
...
export async function handleTop(message) {
    const date = todayStr();
    ...
    const lines = [`🏆 **Лидерборд за ${date} (МСК)**`];
    if (pending.length) {
    ...
    const dist = Object.fromEntries(Array.from({ length: 20 }, (_, i) => [i + 1, 0]));
    normal.forEach((r) => { if (r.value >= 1 && r.value <= 20)
        dist[r.value]++; });
    lines.push('');
    ...
    if (st) {
        ...
        lines.push('');
        lines.push(`⚔️ **Активный тай-брейк** за ${st.date} (раунд ${st.round}): ${names}. Дедлайн: **${deadlineStr}**`);
    }

...
import { meta, rolls, serverRoles, tiebreaks } from './schema.js';
// ─── meta ────────────────────────────────────────────────────────────────────
...
export async function getServerRoleId(roleType) {
    const [row] = await db
        .select()
        .from(serverRoles)
...
export async function setServerRoleId(roleType, roleId) {
    await db
        .insert(serverRoles)
...
export async function getAllServerRoles() {
    const rows = await db.select().from(serverRoles);
    const map = { arbuz: null, tykvenets: null };
    for (const row of rows)
        map[row.roleType] = row.roleId;
    return map;
}
// ─── rolls ───────────────────────────────────────────────────────────────────
export async function getUserRoll(date, userId) {
    const [row] = await db
        .select()
        .from(rolls)
...
export async function insertRoll(date, userId, value, mode, finalized) {
    await db.insert(rolls).values({ date, userId, value, mode, finalized });
}
export async function updateRoll(date, userId, value, mode, finalized) {
    await db
        .update(rolls)
...
export async function finalizePendingAsOnes(date) {
    await db
        .update(rolls)
        .set({ finalized: true, mode: 'normal', value: 1 })
        .where(and(eq(rolls.date, date), eq(rolls.finalized, false)));
}
export async function getDayRolls(date) {
    return db.select().from(rolls).where(eq(rolls.date, date));
}
/** Возвращает true, если в анархии за этот день кто-то выбил 20 */
export async function anarchyTwentyBlocksRole(date) {
    const [result] = await db
        .select({ total: count() })
        .from(rolls)
        .where(and(eq(rolls.date, date), eq(rolls.finalized, true), eq(rolls.mode, 'anarchy'), eq(rolls.value, 20)));
    return (result?.total ?? 0) > 0;
}
/** Возвращает всех участников с максимальным обычным броском за день */
export async function topCandidates(date) {
    const rows = await db
        .select({ userId: rolls.userId, value: rolls.value })
        .from(rolls)
        .where(and(eq(rolls.date, date), eq(rolls.finalized, true), eq(rolls.mode, 'normal')))
...
// ─── tiebreaks ───────────────────────────────────────────────────────────────
export async function tiebreakUserAlreadyRolled(date, round, userId) {
    const [row] = await db
        .select({ id: tiebreaks.userId })
        .from(tiebreaks)
        .where(and(eq(tiebreaks.date, date), eq(tiebreaks.round, round), eq(tiebreaks.userId, userId)));
    return row !== undefined;
}
export async function tiebreakRecord(date, round, userId, value) {
    await db.insert(tiebreaks).values({ date, round, userId, value });
}
export async function tiebreakRoundResults(date, round) {
    return db

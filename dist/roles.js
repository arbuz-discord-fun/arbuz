...
async function removeRoleFromAll(role) {
...
export async function setWinnerRole(winnerId) {
    const guild = getGuild();
    if (!guild)
        return false;
 ...
 */
export async function setTykvenetsToAll() {
    const guild = getGuild();
    if (!guild)
        return false;
    ...
    if (arbuzRole)
        await removeRoleFromAll(arbuzRole);
    let members;
    try {
...
export function getDisplayName(guild, userId) {

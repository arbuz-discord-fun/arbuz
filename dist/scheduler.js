 ...
 */
function msUntilDayClose() {
    const now = Date.now();
    const msk = nowMsk(); // getUTC* методы дают МСК значения
    let target = new Date(Date.UTC(msk.getUTCFullYear(), msk.getUTCMonth(), msk.getUTCDate(), 20, 59, 0, 0)).getTime();
    if (target <= now)
